...
#define F first
#define S second
int n,m,x,y;vi euler,height,first,t;vvi a;
...
void solve(){
  cin>>n;vi has_parent(n+1,0);height=first=vi(n+1,0);euler.clear();a=vvi(n+1);
  rep(i,1,n){
    cin>>m;
    while(m--){
      cin>>x;
  ...
  int root=1;
  rep(i,1,n)if(has_parent[i]^1){root=i;break;}
  dfs(root);t=vi(euler.size()*4,0);
  build();
  cin>>m;
  rep(i,1,m){
    cin>>x>>y;x=first[x],y=first[y];
    if(y<x)swap(x,y);
    cout<<sum(x,y);
    if(i!=m)cout<<"\n";
  }
