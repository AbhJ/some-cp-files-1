...
void solve(){
    ...
    //     cout<<"\n";
    // }
   int n;cin>>n;bool b[n+2],x[n+2];memset(b,0,sizeof(b));std::vector<int> v(n);
   for (int i = 0; i < n; ++i)cin>>v[i];
   int mi=0,ma=0,mi2=0,ma2=0;
   sort(v.begin(), v.end());int l=-1e9;
   for (int i = 0; i < n; ++i)
   {
       if(v[i]>l+2)mi++,l=v[i];
   }
   ...
   for (int i = 0; i < n; ++i)
   {
       if(max(v[i]-1,l+1)<=v[i]+1)ma++,l=max(v[i]-1,l+1);
   }
   cout<<mi<<" "<<ma;
}
int main()
