...
//     return c+1;
// }
ll solver(std::vector<ll> v,ll n){
    if(v.empty())return 0;
    if(n<0)return 0;
    std::vector<ll> on,off;
    for (auto i:v)
    {
        if(bit_set(i,n))on.pb(i);
        else off.pb(i);
    }
    ll onc=solver(on,n-1),ofc=solver(off,n-1);
    if(!on.size())return(ofc);
    if(!off.size())return(onc);
    return min(onc,ofc)+(1LL<<n);
}
void solve(){
    // cin>>n;
    // for (int i = 0; i < n; ++i)
