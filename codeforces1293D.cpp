...
void solve(){
    ...
    //     cout<<"\n";
    // }
   ll x,y,ax,ay,bx,by,xs,ys,t;cin>>x>>y>>ax>>ay>>bx>>by>>xs>>ys>>t;
   ll L=1LL<<62-1;
   std::vector<pair<ll,ll>> v;v.pb({x,y});
   while(((L-bx)/ax>=x) and ((L-by)/ay>=y )){
        x=ax*x+bx,y=ay*y+by;
        v.pb({x,y});
   }
   ll ans=0;
   for (ll i=0;i<v.size();i++)
   {
       for(ll j=i;j<v.size();j++){
         ll d0=abs(v[i].F-v[j].F)+abs(v[i].S-v[j].S);
         ll d1=abs(v[i].F-xs)+abs(v[i].S-ys);
         ll d2=abs(xs-v[j].F)+abs(ys-v[j].S);
         if(t>=d0+d1)ans=max(ans,j-i+1);
         if(t>=d0+d2)ans=max(ans,j-i+1);
       }
   }
   cout<<ans;
}
