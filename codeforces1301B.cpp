...
void solve(){
    ll n;cin>>n;ll ma=-1000000010,mi=1000000010;std::vector<ll> ori(n);
    for (int i = 0; i < n; ++i)cin>>ori[i];
        for(int i = 0;i<n;i++){
        if(i > 0 && ori[i] == -1 && ori[i - 1] != -1)
            mi = min(mi , ori[i - 1]) , ma = max(ma , ori[i - 1]);
        if(i < n - 1 && ori[i] == - 1 && ori[i + 1] != -1)
            mi = min(mi , ori[i + 1]) , ma = max(ma , ori[i + 1]);
    }
    ll av=(ma+mi)/2,m=0;
    // for (int i = 0; i < v.size(); ++i)
    // {
    //     av+=v[i];/*if(i)m=max(abs(v[i]-v[i-1]),m);*/
    // }
    // if(v.size())av/=v.size();
    for (int i = 0; i < n; ++i)
    {
        if(ori[i]==-1)ori[i]=av;
        if(i)m=max(abs(ori[i]-ori[i-1]),m);
    }
    // if(m==-1e9)m=0;
    cout<<m<<" "<<av;
}
