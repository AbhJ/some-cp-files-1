...
void solve(){
    ...
    //     cout<<"\n";
    // }
    string s,t;cin>>s;set<char>g[10000];std::vector<char> v;
    int n=s.length();
    for (int i = 0; i < n-1; ++i)
        g[(int)s[i]].insert(s[i+1]),g[(int)s[i+1]].insert(s[i]);
    for (char i = 'a'; i <= 'z'; ++i)
    {
        if(g[(int)i].size()>2){cout<<"NO\n";return;}
        if(g[(int)i].size()==1)v.pb(i);//can be placed at corner
    }
    if(v.size()!=2 and n>1){cout<<"NO\n";return;}
    if(n>1){
        char cur=v[0],last='.';
        while(1){
            t+=cur;
            if(cur==v[1])break;
            g[(int)cur].erase(last);
            auto i=g[(int)cur].begin();//prints adjacent element
            last=cur;
            cur=*i;
        }
    }
