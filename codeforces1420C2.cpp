...
void solve(){
  ...
  int sum=0,x=1;
  rep(i,2,n){
      if(a[i]<a[i-1] and x)sum+=a[i-1],x^=1;
      else if(a[i]>a[i-1] and x^1)sum-=a[i-1],x^=1;
  }
  cout<<(sum=sum+(x?a[n]:0))<<"\n";
  while(q--){
      cin>>l>>r;
      set<int>s;
      s.insert(l-1),s.insert(l),s.insert(r),s.insert(r+1),s.insert(r-1),s.insert(l+1);
      for(auto i:s){
          if(a[i]>a[i-1] and a[i]>a[i+1])sum-=a[i];
          if(a[i]<a[i-1] and a[i]<a[i+1])sum+=a[i];
      }
      swap(a[l],a[r]);
      for(auto i:s){
